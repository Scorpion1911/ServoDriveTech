...
int16 CServoDriverCom::GTSD_CMD_SetServoOn(int16 axis)
{
    ...
    int16 dsp_comAddr = RN_USER_PROTOCOL_DRIVER;						//???
    if (m_pDriver == NULL)
    {
    ...
    dspdata[0] = GetCmdIDAndAxisNum(cmd_id, m_pMapping->ConvertAxiToSubAxiId(axis));					//????????????id
...
int16 CServoDriverCom::GTSD_CMD_SetWaveBuf(int16 dsp_number, WAVE_BUF_PRM wave)
{
    ...
    dspdata[0] = GetCmdIDAndAxisNum(cmd_id, 0);					//????????????id
    dspdata[1] = GTSD_DSP_WRITE;									//§Υ????
    dspdata[2] = 0;												//?????
...
int16 CServoDriverCom::GTSD_CMD_ProcessorGeneralFunc(int16 axis, GENERALFUNCTION* gefunc)
{
    ...
    gefunc->data[0] = GetCmdIDAndAxisNum(gefunc->cmd, m_pMapping->ConvertAxiToSubAxiId(axis));						//????????????id
