...
public:
    ...
    Uint8               m_conflict_device_seq;  //conflict device sequence number,not station
...
// 	short(*UserHandleRespFunctionDriver)(StUserPacket* packet);
    ...
    CUserResp *m_pUserHandleRespFunctionPlot;
...
protected:
    ...
    virtual short RxPacketDecoder(const void* src, const Uint16 src_len, void* des, Uint16& des_len);//????????????????????§ß??§ß??????¨²????????ûy??
