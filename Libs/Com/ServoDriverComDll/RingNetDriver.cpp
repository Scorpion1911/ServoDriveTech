...
// 	srand(time(0));/*设置种子,并生成伪随机序列*/
	...
	m_pUserHandleRespFunctionPlot = NULL;
}
...
//	if (pPacket->ringnet_head.src_id == RN_FPGA_CH_ID) //??? 代码有误，但是已经想不起原来需要表达的意思。
	if (pUser_payload_head->des_ch == RN_FPGA_CH_ID)
	{
	...
	switch (pUser_payload_head->start_addr_bit7_0)
	{
	case RN_USER_PROTOCOL_DRIVER:
	...
	case RN_USER_PROTOCOL_PC:
		...
		break;
	case RN_USER_PROTOCOL_PLOT:
		if (m_pUserHandleRespFunctionPlot != NULL)
		{
			rtn = m_pUserHandleRespFunctionPlot->UserHandleRespFunction(&user_packet);
			if (rtn != RTN_SUCCESS)
			{
				return rtn;
			}
