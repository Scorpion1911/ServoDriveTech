...
{
	for (int32 i = 0; i <= int32(byte_num - sizeof(FRAME_KEYWORD)); i++)
	{
...
{
	int32 i;
	for (i = 0; i <= int32(byte_num - m_size_of_frame); i = i + m_size_of_frame)
	{
...
// 			i = i;
	...
	left_byte_num = (Uint16)(byte_num - i);
	return RTN_SUCCESS;
}
//------------------------------------------------------
...
//	bool find_frame_key = false;
	...
	{
		if (m_pDataTmp_valid_num > sizeof(FRAME_KEYWORD)-1)
		{
...
//number实际取到的数据，数据的指针
short CRnDriverWave::PW_PcGetWaveBufferData(char* data, Uint32& number)// number means point
{
	Uint32 max_frame_num = number / m_size_of_frame;
	if (data == NULL)
	{
		return RTN_NULL_POINT;
	}
	...
	ParseDspWaveData(m_dsp_id, 0, NULL);
	EnterCriticalSection((CRITICAL_SECTION*)m_critical_section);
	//////////////////////////////////////////////////////////////////////////
	Uint32 valid_frame_num = m_frame_write_cnt - m_frame_read_cnt;
	if (valid_frame_num >= MAX_FRAME_NUM)
...
//	Uint16 byte_cnt;
	...
	{
		read_frame_index_byte_offset = ((read_beg_index + i) % MAX_FRAME_NUM) * m_size_of_frame;
		memcpy(data, m_pFrameBuffer + read_frame_index_byte_offset, m_size_of_frame);
		data += m_size_of_frame;
	}

	m_frame_read_cnt += number;
	number = number * m_size_of_frame;
	//返回读取的个数
	LeaveCriticalSection((CRITICAL_SECTION*)m_critical_section);
#ifdef TIME_TEST
