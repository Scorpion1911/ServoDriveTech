...
long CRnDriverPlot::PW_PcGetDspWaveBufferSize(Uint16 dsp_station_id)
{
	for (int i = 0; i < m_wave_num; i++)
	{
		if (dsp_station_id == m_pWaveList[i].m_dsp_id)
		{
			return m_pWaveList[i].PW_PcGetDspWaveBufferSize();
		}
	}
	return 0;
}
...
int16 CRnDriverPlot::PW_PcGetWaveBufferData(Uint16 dsp_station_id, char* data, Uint32& number)
{
	for (int i = 0; i < m_wave_num; i++)
	{
		if (dsp_station_id == m_pWaveList[i].m_dsp_id)
		{
			return m_pWaveList[i].PW_PcGetWaveBufferData(data, number);
		}
	}
	return RTN_OBJECT_UNCREATED;
}
