...
protected:
	...
	Uint16  m_wave_num;//m_driver_wave_num;//here the m_wave_num means CRnDriverWave Num

public:
	int16 PW_CreateDspWave(Uint16 dsp_num/*wave_num*/, Uint16* dsp_id_list);
	long PW_PcGetDspWaveBufferSize(Uint16 dsp_station_id);
	int16 PW_PcGetDspWaveData(Uint16 dsp_station_id, double** data, Uint32& number);	// 	//从DSPA或者是DSPB获取数据
	int16 PW_PcGetWaveBufferData(Uint16 dsp_station_id, char* data, Uint32& number);
