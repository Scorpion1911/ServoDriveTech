#include "encconfigsinhiperface.h"
...
EncConfigSinHiperface::EncConfigSinHiperface(QObject *parent) : IEncConfigItem(parent)
{
    m_encType = ENC_TYPE_SIN;
    m_encConfigData = 0x0008;
    m_comboType = 2;
    m_isLineNumEditable = false;
    m_adGain = 4;
    m_sinConf = 8;
    m_needTimer = true;
    setObjectName(tr("SinHiperface"));
}

EncConfigSinHiperface::~EncConfigSinHiperface()
{
    GT::deletePtrObject(m_attributeUi);
}

void EncConfigSinHiperface::createAttributeUi()
{
...
void EncConfigSinHiperface::updateAttributeUi()
{
...
bool EncConfigSinHiperface::execute()
{
    m_encConfigData=0x0008;
    m_comboType = 2;
    m_adGain = 4;
...
void EncConfigSinHiperface::setAbsPos(quint32 value)
{
    m_absPos->setValue(value);
}

void EncConfigSinHiperface::onSinNumChanged(int num)
{
